    ...
    <View className=" bg-white flex-1">
                ...
                Object.entries(groupedItems).map(([key, items])=>{
                    return (
                        ...
                        className="flex-row items-center space-x-3 py-2 px-4 bg-white rounded-3xl mx-2 mb-3 shadow-md">
                            <Text style={{color: themeColors.text}} className="font-bold mr-2">{items.length} x </Text>
                            <Image className="h-14 w-14 rounded-full" source={{uri: urlFor(items[0]?.image).url()}} />
                            <Text className="flex-1 font-bold text-gray-700 ml-2">{items[0]?.name}</Text>
                            <Text className="font-semibold text-base mr-2">${items[0]?.price}</Text>
                            <TouchableOpacity
     ...
     {/* totals */}
      <View style={{backgroundColor: themeColors.bgColor(0.2)}} className=" p-6 px-8 rounded-t-3xl space-y-4">
            ...
            <View className="flex-row justify-between">
                <Text className="font-extrabold mb-2">Order Total</Text>
